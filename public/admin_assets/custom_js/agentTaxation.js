...
$("#save_form").on("submit",function(e){
  ...
  $.ajaxSetup({
	...
	$.ajax({ 
		type:"post",
		url:siteUrl + "/agent-taxation/save",  
...
function dataTable(){
	...
	var table = $('#cat_datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: siteUrl+"/agent-taxation/show",
        columns: [ 
            {data: 'DT_RowIndex'},
            {data: 'name', name: 'name'},
...
function deletes(id = ''){
    if (confirm("Are you sure!")) {
        $.ajax({
            url:siteUrl+"/agent-taxation/delete",
            type:'GET',
            data:{id:id},
...
function edit(id = '') {
    $.ajax({
        url: siteUrl +"/agent-taxation/edit",
        data: { id: id },
        type: 'GET',
        dataType: "json",
        success: function (res) {
            ...
            $("#ref_name").val(res.data.ref_name);
            ...
            $("#insurent_payout").val(res.data.insurent_payout);
            $("#agent_payout").val(res.data.agent_payout);
            $("#agent_value").val(res.data.agent_value);
            $("#submit").text("Update");
            // reset();
...
function status(id = "", status = "") {
    $.ajax({
        url: siteUrl +"/agent-taxation/status",
...
function insurentValcal(obj){
    var val = $('#premium').val();
    var per_val = $('#agent_value').val();
    var perc =(Number(val) * Number(per_val)/100) ;
    var insurentVal =(Number(val) - Number(perc)) ;
    $('#agent_payout').val(perc);
    $('#insurent_payout').val(insurentVal);

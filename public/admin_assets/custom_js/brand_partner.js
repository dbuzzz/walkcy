...
$("#subcategory_form").on("submit",function(e){
  ...
  $.ajaxSetup({
	...
	$.ajax({ 
		type:"post",
		url:siteUrl + "/brand_partner/save",  
...
function show_project(){
	...
	var table = $('#cat_datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: siteUrl+"/brand_partner/show",
        columns: [ 
            {data: 'DT_RowIndex'},
...
function deletes(id = ''){
    if (confirm("Are you sure!")) {
        $.ajax({
            url:siteUrl+"/brand_partner/delete",
            type:'GET',
            data:{id:id},
...
function edit(id = '') {
    $.ajax({
        url: siteUrl +"/brand_partner/edit",
        data: { id: id },
        type: 'GET',
        dataType: "json",
        success: function (res) {
...
function status(id = "", status = "") {
    $.ajax({
        url: siteUrl +"/brand_partner/status",

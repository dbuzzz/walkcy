...
function show_project(){
	...
	var table = $('#cat_datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: siteUrl+"/sub-category/show",
        columns: [ 
            {data: 'DT_RowIndex'},
            {data: 'name', name: 'name'},
...
function deletes(id = ''){
    if (confirm("Are you sure!")) {
        $.ajax({
            url:siteUrl+"/sub-category/delete",
            type:'GET',
            data:{id:id},
...
function status(id = "", status = "") {
    $.ajax({
        url: siteUrl +"/sub-category/status",

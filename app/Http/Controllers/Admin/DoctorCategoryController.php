...
class DoctorCategoryController extends Controller
{
    ...
    {
      return view('admin.doctor_category');
    }//end of method
    ...
    public function save(Request $request){
        ...
        if($validator->passes()) {
            ...
            $formdata['commision'] = $request->commision;
            ...
            if (!empty($id)) {
                ...
                $row = DoctorCategory::where('id',$id)->update($formdata);
                $msg = 'Data Updated';
            }else{
                $formdata['added_by'] = Auth::user()->id;
                $row = DoctorCategory::insertGetId($formdata);
                $msg = 'Data Inserted';
    ...
    public function show(Request $request){
        if ($request->ajax()) {
            $data = DoctorCategory::where('is_deleted',0)->orderBy('id','DESC')->get();
    ...
    public function edit(request $request){
        $data = DoctorCategory::where('id',$request->id)->first();
        // dd($data);
        if(empty($data)){
    ...
    public function delete(Request $request){
        ...
        $row = DoctorCategory::where('id',$request->id)->update($data);

...
class GenralPageController extends Controller
{
    public function index()
    {
      return view('admin.genralpage');
    }//end of method
    ...
    public function save(Request $request){
        ...
        if($validator->passes()) {
            ...
            if (!empty($id)) {
                ...
                $row = GenralPage::where('id',$id)->update($formdata);
                $msg = 'Data Updated';
            }else{
                $formdata['added_by'] = Auth::user()->id;
                $row = GenralPage::insertGetId($formdata);
                $msg = 'Data Inserted';
    ...
    public function show(Request $request){
        if ($request->ajax()) {
            $data = GenralPage::where(['is_deleted'=>0])->orderBy('id','DESC')->get();
    ...
    public function edit(request $request){
        $data = GenralPage::where('id',$request->id)->first();
        // dd($data);
        if(empty($data)){
    ...
    public function delete(Request $request){
        ...
        $row = GenralPage::where('id',$request->id)->update($data);

...
use App\Models\{Attribute,Role,Form};
use Validator;
...
class FormController extends Controller
{
    public function index()
    {
      ...
      $data['attribute'] = Attribute::where(['is_deleted'=>0,'active_status'=>1])->get();
    ...
    public function save(Request $request){
        ...
        if($validator->passes()) {
            ...
            $formdata['role_id'] = $request->role;
            $formdata['attribute_id'] = implode(',', $request->attribute);
            if (!empty($id)) {
                $formdata['updated_by'] = Auth::user()->id;
                $row = Form::where('id',$id)->update($formdata);
                $msg = 'Data Updated';
            }else{
                $formdata['added_by'] = Auth::user()->id;
                $row = Form::insertGetId($formdata);
                $msg = 'Data Inserted';
    ...
    public function show(Request $request){
        if ($request->ajax()) {
            $data = Form::join('roles','roles.id','=','forms.role_id')->where(['forms.is_deleted'=>0])->orderBy('forms.id','DESC')->get(['forms.id','forms.active_status','forms.title','roles.name',]);
    ...
    public function delete(Request $request){
        ...
        $row = Form::where('id',$request->id)->update($data);

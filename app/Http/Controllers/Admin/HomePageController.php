...
use App\Models\{HomePage,Category};
use Validator;
...
class HomePageController extends Controller
{
    ...
    public function save(Request $request){
        ...
        if($validator->passes()) {
            ...
            if (!empty($id)) {
                ...
                $row = HomePage::where('id',$id)->update($formdata);
                $msg = 'Data Updated';
            }else{
                $formdata['added_by'] = Auth::user()->id;
                $row = HomePage::insertGetId($formdata);
                $msg = 'Data Inserted';
    ...
    public function show(Request $request){
        if ($request->ajax()) {
            $data = HomePage::join('categories','categories.id','=','home_pages.cat_id')->where(['home_pages.is_deleted'=>0,'categories.is_deleted'=>0])->orderBy('home_pages.id','DESC')->get(['home_pages.id','home_pages.active_status','home_pages.heading','home_pages.sub_heading','home_pages.banner','home_pages.image1','home_pages.image2','categories.name as category']);
    ...
    public function edit(request $request){
        $data = HomePage::where('id',$request->id)->first();
        // dd($data);
        if(empty($data)){
    ...
    public function delete(Request $request){
        ...
        $row = HomePage::where('id',$request->id)->update($data);

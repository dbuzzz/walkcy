...
class RoleController extends Controller
{
    ...
    public function save(Request $request){
        ...
        if($validator->passes()) {
            ...
            $formdata['name'] = $request->role;
            if ($request->create) {
                $formdata['create'] = $request->create;
            }else{
                $formdata['create'] = 0;
            }
            if ($request->update) {
                $formdata['update'] = $request->update;
            }else{
                $formdata['update'] = 0;
            }
            if ($request->delete) {
                $formdata['delete'] = $request->delete;
            }else{
                $formdata['delete'] = 0;
            }
            ...
            if (!empty($id)) {
                ...
                $row = Role::where('id',$id)->update($formdata);
                $msg = 'Data Updated';
            }else{
                $formdata['added_by'] = Auth::user()->id;
                $row = Role::insertGetId($formdata);
                $msg = 'Data Inserted';
    ...
    public function delete(Request $request){
        ...
        $row = Role::where('id',$request->id)->update($data);

...
class CategoryController extends Controller
{
    ...
    public function save(Request $request){
        ...
        if($validator->passes()) {
            ...
            if (!empty($id)) {
                ...
                $row = Category::where('id',$id)->update($formdata);
                $msg = 'Data Updated';
            }else{
                $formdata['added_by'] = Auth::user()->id;
                $row = Category::insertGetId($formdata);
                $msg = 'Data Inserted';
    ...
    public function show(Request $request){
        if ($request->ajax()) {
            $data = Category::where('is_deleted',0)->orderBy('id','DESC')->get();
    ...
    public function delete(Request $request){
        ...
        $row = Category::where('id',$request->id)->update($data);

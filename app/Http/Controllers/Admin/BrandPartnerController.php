...
use App\Models\{BrandPartner,Category};
use Validator;
...
class BrandPartnerController extends Controller
{
    ...
    {
      return view('admin.brand_partner');
    }//end of method
    ...
    public function save(Request $request){
        ...
        if($validator->passes()) {
            ...
            if (!empty($id)) {
                ...
                $row = BrandPartner::where('id',$id)->update($formdata);
                $msg = 'Data Updated';
            }else{
                $formdata['added_by'] = Auth::user()->id;
                $row = BrandPartner::insertGetId($formdata);
                $msg = 'Data Inserted';
    ...
    public function show(Request $request){
        if ($request->ajax()) {
            $data = BrandPartner::where(['is_deleted'=>0])->get();
    ...
    public function edit(request $request){
        $data = BrandPartner::where('id',$request->id)->first();
        // dd($data);
        if(empty($data)){
    ...
    public function delete(Request $request){
        ...
        $row = BrandPartner::where('id',$request->id)->update($data);
